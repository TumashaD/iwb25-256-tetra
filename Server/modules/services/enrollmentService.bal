...
type EnrollmentTeamMember record {
    string id;
    string name;
    string email;
    string role;
...
public function createEnrollmentService(postgresql:Client dbClient, http:CorsConfig corsConfig, auth:AuthInterceptor authInterceptor) returns http:InterceptableService {
    return @http:ServiceConfig {cors: corsConfig} isolated service object {
        ...
        `;
            ...
            record {int count;}[]|error leadershipArr = from record {int count;} c in leadershipResult
                select c;
        ...
        `;
            ...
            record {int count;}[]|error existingArr = from record {int count;} c in checkResult
                select c;
        ...
        `;
            ...
            Enrollment[]|error enrollments = from Enrollment e in enrollmentResult
                select e;
        ...
        `);
            ...
            Enrollment[]|error updatedEnrollmentArr = from Enrollment e in updatedEnrollmentResult
                select e;
        ...
        // Get member role , user name, email, avatar url
        isolated resource function get members/[int enrollmentId]() returns EnrollmentTeamMember[]|http:NotFound|http:InternalServerError|error {
            sql:ParameterizedQuery query = `
            SELECT u.id, u.name, u.email, tm.role
                FROM 
                team_members tm JOIN users u ON tm.member_id = u.id
                WHERE tm.team_id = (
                    SELECT team_id from enrollments WHERE enrollment_id = ${enrollmentId}
                )
        `;

            stream<EnrollmentTeamMember, sql:Error?> teamMembers = self.db->query(query, EnrollmentTeamMember);

            EnrollmentTeamMember[]|error members = from EnrollmentTeamMember m in teamMembers
                select m;

            if members is error {
                log:printError("Failed to fetch enrollment team members", members);
                return http:INTERNAL_SERVER_ERROR;
            }
